...
DragHandler.getCurTarget = function()	
{ 
	return dragTarget; 
};
DragHandler.getCurTargetHandle = function() { return dragTargetHandle; };
...
{
	var elToClone = (targetHasHandle) ? event.target.parentNode : event.target;
...
DragHandler.end = function(event, curDropzone)
{
	// swap elements if we're dropping over a similar element
	if(startTarget && curDropzone && startTarget.nodeName === curDropzone.nodeName) 
	{
...
DragHandler.getCurDropzone = function(event, curTarget)
{
  if(!curTarget) { return null; }

  // make sure we only drop in dropzones with the same element
  var nodeName = curTarget.nodeName.toLowerCase();
  ...
  for(var i=0; i < numDropzones; i++) 
  {
    ...
    // if we find the dropzone, curDropzone is the current dropzone
  ...
  return (isDropzone == true) ? curDropzone : null;
};
